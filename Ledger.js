...
app.post('/api/v1/CreateLedger', async (req, res) => {
  ...
  try {
    const Internal_authentication_key = req.headers['internal_authentication_key'];
    const { FirstName, LastName, DOB, email, userid } = req.body;

    // Validate user details 
    if (!FirstName || !LastName || !DOB || !email || !userid) {
      return res.status(400).send({ message: 'FirstName, LastName, DOB, email and userid are required' });
    }
...
const authentication_key = await client.db("Ledger").collection("Authentication_keys").findOne({ Internal_authentication_key: Internal_authentication_key });    if (!authentication_key) {
...
const authenticationlog = await client.db("Ledger").collection("Authentication_log").insertOne({
  Ledgerid: Ledgerid,
  action: 'Create',
  ...
  action_at: new Date(),
    ...
    res.status(201).send({ message: 'Ledger created successfully', Ledgerid: Ledgerid });
  } catch (error) {
    console.error(error);
...
app.post('/api/v1/LogLedger', async (req, res) => {
  ...
  try {
    const Internal_authentication_key = req.headers['internal_authentication_key'];
    const { userid, amount, credit, debit, transaction_id, cardUsed} = req.body;
    const authentication_key = await client.db("Ledger").collection("Authentication_keys").findOne({ Internal_authentication_key: Internal_authentication_key });    if (!authentication_key) {
      return res.status(400).send({ message: 'Invalid authentication key' });
    }
    // Validate user details 
    if (!userid || !amount || (credit == null) || (debit == null)) {
      return res.status(400).send({ message: 'userid, amount, credit, and debit are required' });
    }
    if (credit && debit) {
      return res.status(400).send({ message: 'credit and debit cannot be true at the same time' });
    }
    if (!credit && !debit) {
      return res.status(400).send({ message: 'credit and debit cannot be false at the same time' });
    }


    const user = await client.db("Ledger").collection("Accounts").findOne({ userid: userid });
    if (!user) {
...
if (debit == true) {
  if (user.balance < amount) {
    ...
    if (credit) {
      await client.db("Ledger").collection("Accounts").updateOne({ userid: userid }, { $inc: { balance: amount } });
    } else if (debit) {
      await client.db("Ledger").collection("Accounts").updateOne({ userid: userid }, { $inc: { balance: -amount } });
    }
    ...
    Ledgerid = uuidv4();
const LedgerLog = await client.db("Ledger").collection("Ledger_log").insertOne({
  Ledgerid: Ledgerid,
  userid: userid,
  amount: amount,
  credit: credit,
...
//Posting Back to The Card Controller
    ...
    return res.status(201).send({ message: 'Transaction logged successfully', Ledgerid: Ledgerid });
  } catch (error) {
    console.error(error);
...
app.post('/api/v1/AddService', async (req, res) => {
  ...
  try {
    ...
    new_key = uuidv4();
const Key = await client.db("Ledger").collection("Authentication_keys").insertOne({
...
app.put('/api/v1/UpdateService', async (req, res) => {
  ...
  try {
    ...
    new_key = uuidv4();
const Key = await client.db("Ledger").collection("Authentication_keys").updateOne(
